...
%     MaxNeuron_saver = MaxNeuron_saver + 4;
    ...
    RBFoutputs = net(AllNewCombinations(:,1:end-1)')';
    aa = find(RBFoutputs(:,end)<0);
    RBFoutputs(aa, end) = 0;
    RBFoutputs = RBFoutputs';
    ...
    R555=corr(MLPoutputs', RBFoutputs')^2
%     disp([R111 R222 R444])
...
% RBFoutputsnormal=(RBFoutputs-min(RBFoutputs))/(max(RBFoutputs)-min(RBFoutputs));
% PlotResults(MLPoutputs,RBFoutputs,MLPoutputsnormal,RBFoutputsnormal,'RBF Examinations');
